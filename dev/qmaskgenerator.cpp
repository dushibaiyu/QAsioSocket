...
bool Url::fromString(const QString &url, Url & mUrl)
{
    int i = url.indexOf(QStringLiteral("://"));
    if (i == -1) return false;

    mUrl.protocol = url.left(i).toLower();
    QString host = url.right(url.size() - i -3);
    i = host.indexOf(QStringLiteral("/"));
    if (i != -1) {
        mUrl.resourceName = host.right(host.size() - i);
        host = host.left(i);
    } else {
        mUrl.resourceName = QStringLiteral("/");
    }
    i = host.indexOf(QStringLiteral("::"));
    if (i != -1) {
        i = host.right(url.size() - i - 2).toInt();
        if (i > 0)
            mUrl.port = static_cast<qint16>(i);
        else
            mUrl.port = -1;
        mUrl.host = host.left(i);
    } else {
        mUrl.host = host.toLower();
        mUrl.port = -1;
    }
    return true;
}
